...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.label_1 = customtkinter.CTkLabel(master=self, text="BM Byggeindustri A/S", text_font=("Samsung Sharp Sans", -30))  # font name and size in px
        self.label_1.place(anchor='c', x=400, y=100)

        self.label_2 = customtkinter.CTkLabel(master=self, text="Production Planning Software v1.3", text_font=("Samsung Sharp Sans", -15))  # font name and size in px
    ...
    def documentation(self, results, days, document):
        ...
        p = document.add_paragraph('Denne plan er vejledende ift. produktionstider og stationsstyring. Sedlen påsættes det nedenfor angivede modul.')
        ...
        production_stations = ['Gips (t.o.m. '+str(tom_values[0])+')',
                               ...
                               'Kloakering (t.o.m. '+str(tom_values[4])+')',
                               'Installationer (t.o.m. '+str(tom_values[5])+')',
       ...
       # print(production_records)
        ...
        string = ('Denne seddel tilhører modul: ' + str(results[8]))

        moduleText = document.add_paragraph()
        moduleText.add_run(string).bold = True

        string = ('Opstart på modul: ' + str((format_datetime(datetime=starting_date, format='EEEE, d, MMMM, y', locale='da_DK'))).capitalize())
        ...
        for i, qty, id, desc in production_records:
            ...
            row_cells[1].text = str((format_datetime(datetime=starting_dates[i], format='EEEE, d, MMMM, y', locale='da_DK'))).capitalize()
            row_cells[2].text = str(id).capitalize()
            row_cells[3].text = desc

        for row in table.rows:
            row.height = Cm(1.5)
        string = str(results[8][0]) + '_' + str(results[8][1]) + '_' + str(results[8][2])
